...
public:
    ...
    void printLeastTransferPath(QTableWidget *parent, const std::string& sc, const std::string& ec, const int st_time);

    // 输出从起点城市到终点城市，花费最小的线路
    void printLeastMoneyPath(QTableWidget *parent,  const std::string& sc, const std::string& ec, const int st_time);

    // 输出从起点城市到终点城市，总时间最短的线路
    void printLeastTimePath(QTableWidget *parent,const std::string& sc, const std::string& ec, const int st_time);

public:

    std::map <std::string, std::vector<LineNode>> m;  //以城市为关键字存储
    std::map <std::string, LineNode> mp;  // 以班次为关键字存储
    std::vector <std::string> MinNumResult;
    std::vector <std::string> MinTimeResult;
    std::vector <std::string> MinCostResult;
    bool FindMinNumFail;
    bool FindMinTimeFail;
    bool FindMinCostFail;
};
