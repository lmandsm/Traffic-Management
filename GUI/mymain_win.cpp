...
{
    //struct Time start_time(choose_month.toInt(),choose_day.toInt(),choose_hour.toInt(
    //                           ),choose_min.toInt());
    //struct LineNode start_pos(choose_transportation.toStdString(),,choose_src.toStdString(),c)
    if(PreferChoice){//花费少
        //ALGraph::printLeastMoneyPath(choose_src.toStdString(),choose_des.toStdString(),)
        ALGraph::printLeastMoneyPath()
    }else{//耗时短
