...
// 输出从起点城市到终点城市，中转次数最少的路径
    void ALGraph::printLeastTransferPath(const std::string& sc, const std::string& ec, const int st_time) {
...
void ALGraph::printLeastTimePath(const std::string& sc, const std::string& ec, const int st_time) {
    ...
    if (reach_time[ec] == INT_MAX) {
        QMessageBox::warning(nullptr,"警告","未查询到此路线");
    }
    else {
        string now_city = ec;
        while (now_city != sc) {
...
// 输出某个城市到其他各城市的花费最少的路线
    ...
    if (!check_sc_and_ec(sc, ec)) {QMessageBox::critical(parent,"提醒","抱歉,不存在对应路线") ;return;}
           //MinCostFail=true;
        ...
        if (dis[ec] == INF) {
            ...
            //cout<<"没路"<<endl;
            QMessageBox::critical(parent,"提醒","抱歉,并未找到对应的路线");
        }
        else {
            std::reverse(result.begin(), result.end());
                //for (auto lnd : result) {
            ...
            for(int ri=0;i<parent->row();++ri){
                parent->setItem(ri,0,QTableWidgetItem(mp[result.begin()+ri].line_number));
                parent->setItem(ri,1,QTableWidgetItem(mp[result.begin()+ri].start_city_name));
                parent->setItem(ri,2,QTableWidgetItem(mp[result.begin()+ri].end_city_name));
                parent->setItem(ri,3,QTableWidgetItem(mp[result.begin()+ri].start_time));
                parent->setItem(ri,4,QTableWidgetItem(mp[result.begin()+ri].end_time));
                parent->setItem(ri,5,QTableWidgetItem(mp[result.begin()+ri].spend_money));
